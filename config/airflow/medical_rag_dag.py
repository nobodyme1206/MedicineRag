...
def send_notification(**context):
    ...
    📊 数据采集: {collect_stats.get('total_articles', 'N/A') if collect_stats else 'N/A'} 篇
    ⚙️ 数据处理: {process_stats.get('chunk_count', 'N/A') if process_stats else 'N/A'} chunks
    🔢 向量化: {embed_stats.get('total_vectors', 'N/A') if embed_stats else 'N/A'} 向量
    📈 评估分数: {eval_stats.get('overall_score', 'N/A') if eval_stats else 'N/A'}/100
...
with DAG(
    ...
    collect_task = PythonOperator(task_id='collect_pubmed', python_callable=collect_pubmed_data, provide_context=True)
    process_task = PythonOperator(task_id='process_data', python_callable=process_data_spark, provide_context=True)
    embed_task = PythonOperator(task_id='generate_embeddings', python_callable=generate_embeddings, provide_context=True, pool='gpu_pool')
    index_task = PythonOperator(task_id='update_milvus', python_callable=update_milvus_index, provide_context=True)
    notify_task = PythonOperator(task_id='send_notification', python_callable=send_notification, provide_context=True, trigger_rule=TriggerRule.ALL_DONE)
...
with DAG(
    ...
    eval_task = PythonOperator(task_id='run_evaluation', python_callable=run_evaluation, provide_context=True)
    notify_eval = PythonOperator(task_id='notify_evaluation', python_callable=send_notification, provide_context=True)
...
with DAG(
    ...
    start = DummyOperator(task_id='start')
    collect = PythonOperator(task_id='collect_pubmed', python_callable=collect_pubmed_data, provide_context=True)
    process = PythonOperator(task_id='process_data', python_callable=process_data_spark, provide_context=True)
    embed = PythonOperator(task_id='generate_embeddings', python_callable=generate_embeddings, provide_context=True)
    index = PythonOperator(task_id='update_milvus', python_callable=update_milvus_index, provide_context=True)
    evaluate = PythonOperator(task_id='run_evaluation', python_callable=run_evaluation, provide_context=True)
    notify = PythonOperator(task_id='send_notification', python_callable=send_notification, provide_context=True, trigger_rule=TriggerRule.ALL_DONE)
    end = DummyOperator(task_id='end')
